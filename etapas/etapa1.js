...
registerView('e1', {
  ...
  render(c) {
    ...
    <div class="grid-2">
      <h3 class="section-title">Calidad y estructura del dataset</h3>
      ...
      <h3 class="section-title">Distribución de features derivados</h3>
      ...
      ${panel({ tag: 'Feature', q: 'Distribución por PosicionPrincipal (top 15)', canvas: 'e1pos', answer: '<b>Respuesta:</b> las posiciones ST y CB son las más comunes; los porteros (GK) representan una minoría del plantel.' })}
      ${panel({ tag: 'Feature', q: 'Distribución de BrechaPotencial (potential − overall)', canvas: 'e1brecha', answer: '<b>Respuesta:</b> la mayoría de jugadores tiene una brecha cercana a 0 (rango −2 a 2), lo que indica que el potencial proyectado ya fue alcanzado. Solo una minoría tiene brechas positivas grandes (talento sin desarrollar).' })}
      ${panel({ tag: 'Feature', q: 'Distribución de EficienciaFinanciera (valor / salario, clip 0–50)', canvas: 'e1ef', answer: '<b>Respuesta:</b> la distribución está concentrada en el extremo superior (45–50), lo que refleja jugadores con salario muy bajo relativo a su valor de mercado. El clip a 50 evita distorsión por valores extremos (ej. salario ≈ 0).' })}
    </div>`;
    ...
    bar('e1pos', D.e1.posicionDist.map(x => x.k), D.e1.posicionDist.map(x => x.v), { horizontal: true, colorVar: '--brand' });
    bar('e1brecha', D.e1.brechaHist.labels, D.e1.brechaHist.counts, { colorVar: '--teal' });
    bar('e1ef', D.e1.eficienciaHist.labels, D.e1.eficienciaHist.counts, { colorVar: '--violet' });
  },
});
